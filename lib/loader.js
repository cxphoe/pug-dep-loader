...
class PugLoader {
  ...
  traverseNodes(nodes, context, compiledBlocks = {}) {
    ...
    const handlers = {
      include: (node) => {
        ...
        deps = utils.mergeArrays(deps, includeInfo.deps);
        return {
          indent,
          type: 'text',
      ...
      block: (node) => {
        ...
        deps = utils.mergeArrays(deps, blockDeps);
  ...
  /**
   * @returns {{content: string, deps: string[]}}
   */
  retrieveContent(filepath, compiledBlocks = {}) {
    ...
    if (extendsNode !== null) {
      ...
      for (const node of restNodes) {
        ...
        depsOfNodes = utils.mergeArrays(depsOfNodes, blockInfo.deps);
      ...
      return {
        ...
        deps: utils.mergeArrays(deps, depsOfNodes, retrieveRes.deps),
      };
    }
    ...
    return {
      ...
      deps: utils.mergeArrays(deps, depsOfNodes),
    };
  }
}
